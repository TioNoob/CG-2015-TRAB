...
namespace CosmicSimulatorController
{
    public class CosmicController
    {
        ...
        {
            Orbs.Add(new Orb("Earth", 40, 100, new MyPoint(0, 0), new Vector(new MyPoint(0, 0))));
            Orbs.Add(new Orb("Moon", 10, 35, new MyPoint(0, 200), new Vector(new MyPoint(0, 0))));
            //Orbs.Add(new Orb("Moon", 10, 35, new MyPoint(300, 200), new Vector(new MyPoint(0, 0))));
        }
        ...
        private MyPoint CalculatePositionEndPoint(Orb orb1, Orb orb2, double forceIntensity)
        {
            ...
            vector = new Vector(orb1.ActualPosition, orb2.ActualPosition);

            triangleMajorAdjacentCateto = vector.EndPoint.X;
            triangleMajorHypotenuse = Triangles.CalculateHypotenuse(orb1.ActualPosition, orb2.ActualPosition);

            radianTeta = Math.Acos(Triangles.CosTeta(triangleMajorAdjacentCateto, triangleMajorHypotenuse));

            triangleMinorHypotenuse = forceIntensity;
            triangleMinorAdjacentCateto = Triangles.AdjacentCateto(radianTeta, triangleMinorHypotenuse);
            triangleMinorOppositeCateto = Triangles.OppositeCateto(radianTeta, triangleMinorHypotenuse);
